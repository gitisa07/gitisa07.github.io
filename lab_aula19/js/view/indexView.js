...
class IndexView {
    ...
    */
    ...
    createDishes(category, dishList, pedido) {
        ...
        let divPedido = `<h3>Pedido</h3><ul id='pedidoList'></ul><h5  id='totalPedido'>0</h5>`;
    ...
    listPedido(pedido) {
        ...
        listaPedido.forEach((item) => {
            itemsObj.innerHTML += `<li> R$ ${item.price} - ${item.name} <button id="${item.code}" class="btn btn-primary remove">X</button></li>`;
        })
        let totalObj = document.getElementById("totalPedido");
        totalObj.innerHTML = "0";
        if(pedido.total) {
            totalObj.innerHTML = "Total: R$ "+pedido.total;
        }
